...
public class SAMLTokenServiceImpl implements SAMLTokenService {

	private final static String STS_ENDPOINT = "https://services-acpt.ehealth.fgov.be/IAM/Saml11TokenService/Legacy/v1";
    private final static XMLSignatureFactory xmlSignatureFactory;
	
	private CryptoService cryptoService;

	public SAMLTokenServiceImpl(CryptoService cryptoService) {
	...
	public SAMLToken getSAMLToken() throws TechnicalConnectorException {
		...
		final Element assertion = this.getToken(this.cryptoService.getHeaderCredential(), this.cryptoService.getHOKCredential(), attributes, designators, "urn:oasis:names:tc:SAML:1.0:cm:holder-of-key", 24);
		return SAMLTokenFactory.getInstance().createSamlToken(assertion, this.cryptoService.getHOKCredential());
	}
